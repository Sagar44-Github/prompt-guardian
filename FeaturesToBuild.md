...
9	Attack Fingerprinting	🔥🔥🔥🔥	15 min	Sagar
10	Sound + Browser Notification on Block	🔥🔥🔥	10 min	Sai Tej
...
**Feature 9: Attack Fingerprinting – Intense Prompt-Based Build Guide**

This feature assigns every detected attack a unique cryptographic fingerprint identifier, a session identifier, and a similarity cluster label that groups visually different prompts that are semantically the same attack. When judges see attack IDs like `ATK-7f3a2b9c`, session IDs like `SES-4a2f`, and cluster labels like `CLUSTER-JB-01 (Jailbreak Variant)` appearing consistently across the UI, they immediately think "this is a real security product with proper forensic tracking." The implementation uses Python's `hashlib` on the backend to generate deterministic hashes, a clustering system that groups similar attacks by attack type and risk band, and a fingerprint history that detects when the SAME attack payload is seen multiple times across a session (repeat attack detection). This feature touches backend, extension overlay, popup, and dashboard — giving every team member something to build simultaneously.

**Who builds this:** Sagar (backend fingerprinting module + endpoint update) + Sai Tej (overlay fingerprint display) + Prithvi (dashboard fingerprint gallery and repeat attack detection UI) + Poojitha (cluster assignment logic).

**Files to create or modify:**
- Create: `firewall/fingerprinter.py` (cryptographic fingerprint generation, session ID management, cluster assignment, repeat attack detection)
- Modify: `firewall/analyzer.py` (call fingerprinter and add fingerprint fields to returned dict)
- Modify: `app.py` (session ID initialization on startup, pass session context to analyzer)
- Modify: `extension/content.js` (display fingerprint ID, cluster label, and repeat attack warning in overlay)
- Modify: `extension/dashboard.html` (fingerprint gallery card, repeat attacks section)
- Modify: `extension/dashboard.js` (render fingerprint data, group by cluster, show repeat detection)
- Modify: `extension/popup.html` (show session ID and fingerprint count in stats)
- Modify: `extension/popup.js` (populate session ID and unique fingerprint count)

**Overall Logic:**
When a prompt arrives at `/analyze`, the fingerprinter module generates four things. First, a deterministic `attack_fingerprint` hash (`ATK-XXXXXXXX`) by MD5-hashing the lowercased, stripped prompt text — identical prompts always get the same fingerprint. Second, a `session_id` (`SES-XXXX`) that is generated once when Flask starts and shared across all requests in the same server session — this ties all attacks from one usage session together. Third, a `cluster_id` (`CLUSTER-XX-NN`) that groups attacks by type and risk band — all jailbreak attempts with risk >= 70 get the same cluster prefix, helping identify attack families. Fourth, a `repeat_count` integer representing how many times this exact fingerprint has been seen in the current session — if a prompt is submitted twice, the second submission shows `repeat_count: 2` and triggers a "Repeat Attack Detected" warning in the UI. The fingerprint data flows through to the extension overlay (showing the ID and cluster prominently), the dashboard (a gallery of unique fingerprints grouped by cluster), and the popup (session ID displayed like a security badge).

---

### Prompt 1: Create the Fingerprinting Module (firewall/fingerprinter.py)

Copy and paste this exact prompt into **Claude.ai**:

```
You are a cybersecurity backend engineer. Build a complete fingerprinting and clustering module for a Flask-based prompt injection firewall called Prompt Guardian.

Create a single file: firewall/fingerprinter.py

Purpose:
- Generate cryptographic fingerprints for each analyzed prompt.
- Track how many times each fingerprint has been seen in the current server session.
- Assign cluster labels that group semantically related attacks into families.
- Generate a persistent session ID for the current Flask server instance.

Module-level state:
- A module-level dict called FINGERPRINT_REGISTRY: maps fingerprint string → count integer.
  This is reset when Flask server restarts (intentional — it tracks per-session repeats).
- A module-level string SESSION_ID generated ONCE when the module first loads.

Required functions:

1. generate_session_id() -> str
   - Generate a 4-character hex session ID.
   - Format: "SES-" + secrets.token_hex(2).upper()
   - Called once at module load time to set SESSION_ID.
   - Example output: "SES-4A2F"

2. generate_attack_fingerprint(prompt: str) -> str
   - Takes the raw prompt string.
   - Normalizes: strip whitespace, convert to lowercase.
   - MD5-hash the normalized text using hashlib.md5(text.encode()).hexdigest().
   - Take the first 8 characters of the hash.
   - Format: "ATK-" + hash[:8].upper()
   - Example output: "ATK-7F3A2B9C"
   - Deterministic: same prompt ALWAYS produces same fingerprint.

3. assign_cluster(attack_type: str, risk_score: float) -> str
   - Maps attack type and risk band to a cluster label.
   - Risk bands:
       Critical band: risk >= 90 → suffix "C"
       High band: risk >= 70 → suffix "H"
       Medium band: risk >= 40 → suffix "M"
       Low band: risk < 40 → suffix "L"
   - Attack type abbreviations:
       ...
       "Data Extraction" → "DE"
       "Role Override" → "RO"
       "Encoded Injection" → "EI"
       "Indirect Injection" → "II"
       "Multi-Modal Injection" → "MM"
       Any multilingual variant (contains parenthesis like "(Hindi)") → "ML"
       "Unknown" or None → "UN"
   - Cluster number: a fixed mapping of abbreviation + band to a specific 2-digit number:
       JB-C → 01, JB-H → 02, JB-M → 03
       IO-C → 04, IO-H → 05, IO-M → 06
       PE-C → 07, PE-H → 08, PE-M → 09
       DE-C → 10, DE-H → 11
       RO-C → 12, RO-H → 13
       EI-C → 14, EI-H → 15
       II-C → 16, II-H → 17
       ML-C → 18, ML-H → 19, ML-M → 20
       UN-L → 99 (unknown/safe)
       Any unrecognized combination → "UN-00"
   - Format: "CLUSTER-" + abbrev + "-" + band + suffix_number
   - Example: Jailbreak with risk 95 → "CLUSTER-JB-C01"
   - Example: Instruction Override with risk 75 → "CLUSTER-IO-H05"
   - Full human-readable label also returned: "Jailbreak (Critical)" etc.

4. track_fingerprint(fingerprint: str) -> int
   - Looks up fingerprint in FINGERPRINT_REGISTRY.
   - Increments count by 1 (or sets to 1 if first time).
   - Updates FINGERPRINT_REGISTRY.
   - Returns the NEW count (1 = first time, 2+ = repeat).

5. get_repeat_status(repeat_count: int) -> dict
   - If repeat_count == 1: return {"is_repeat": False, "repeat_label": "First occurrence", "repeat_severity": "none"}
   - If repeat_count == 2: return {"is_repeat": True, "repeat_label": "⚠️ Repeat attack (2nd occurrence)", "repeat_severity": "warning"}
   - If repeat_count >= 3: return {"is_repeat": True, "repeat_label": "🔴 Persistent attack ({N}th occurrence)", "repeat_severity": "critical"}

6. get_registry_stats() -> dict
   - Returns summary stats about the current session's FINGERPRINT_REGISTRY:
     {
       "total_unique_fingerprints": len(FINGERPRINT_REGISTRY),
       "total_repeat_attacks": count of fingerprints with count > 1,
       "most_repeated": the fingerprint string with highest count (or None if empty),
       "most_repeated_count": that highest count value,
       "session_id": SESSION_ID
     }

7. fingerprint_prompt(prompt: str, attack_type: str, risk_score: float) -> dict
   - The main entry point function that combines all the above.
   - Calls generate_attack_fingerprint(prompt) → fingerprint.
   - Calls assign_cluster(attack_type, risk_score) → cluster dict.
   - Calls track_fingerprint(fingerprint) → repeat_count.
   - Calls get_repeat_status(repeat_count) → repeat_status.
   - Returns one unified dict:
     {
       "fingerprint": "ATK-7F3A2B9C",
       "session_id": SESSION_ID,
       "cluster_id": "CLUSTER-JB-C01",
       "cluster_label": "Jailbreak (Critical)",
       "cluster_abbrev": "JB",
       "repeat_count": 1,
       "is_repeat": False,
       "repeat_label": "First occurrence",
       "repeat_severity": "none"
     }

Code quality:
- Module docstring explaining the forensic fingerprinting approach.
- Comment explaining WHY MD5 is used here (speed, not security — this is forensic ID generation, not cryptography).
- Thread safety note: for a hackathon single-threaded Flask server, simple dict is fine. In production, use threading.Lock().
- A test block (if __name__ == "__main__") that tests:
  - Same prompt twice should produce same fingerprint and repeat_count of 2.
  - Different prompts produce different fingerprints.
  - Cluster assignment for each attack type.

Return ONLY the complete fingerprinter.py file.
```

---

### Prompt 2: Update analyzer.py to Call the Fingerprinter

Use this prompt for Poojitha to integrate the module:

```
I have an existing firewall/analyzer.py file for Prompt Guardian. It currently calls pattern_check, groq_check, calculate_risk_score, sanitize_prompt, get_safe_version, detect_language, and is_non_english.

Add fingerprinting as the FINAL step before returning the result dict.

Required changes:

1. Add import at the top:
   from firewall.fingerprinter import fingerprint_prompt, get_registry_stats

2. Inside analyze_prompt(prompt: str) -> dict, AFTER all existing steps (after sanitization and building the return dict), add:

   Step 6 (NEW — Fingerprinting):
   - Get attack_type from score_result.get('attack_type', 'Unknown')
   - Get risk_score from score_result.get('risk_score', 0.0)
   - Call fingerprint_result = fingerprint_prompt(prompt, attack_type, risk_score)
   - Call registry_stats = get_registry_stats()

3. Add ALL fields from fingerprint_result to the returned dict using ** unpacking:
   return {
     **score_result,
     # ... existing fields ...
     **fingerprint_result,
     "registry_total_unique": registry_stats["total_unique_fingerprints"],
     "registry_repeat_count": registry_stats["total_repeat_attacks"],
   }

4. The fingerprinting should run for EVERY prompt regardless of action (ALLOW/WARN/BLOCK) so safe prompts also get fingerprints. This lets us detect if someone is probing with subtly different variants that are below detection threshold.

5. Do NOT fingerprint if prompt is empty (it should already be validated in app.py, but add a guard anyway).

Return ONLY the modified sections of analyzer.py (the new import and the new Step 6 code and the updated return dict structure) with clear comments showing exact insertion points.
```

---

### Prompt 3: Initialize Session ID in app.py

Use this prompt for Sagar to update the server:

```
I have an existing app.py Flask file for Prompt Guardian. It currently has /health, /analyze, /threat-feed, and /generate-report endpoints.

Make these small additions:

1. Add import at the top:
   from firewall.fingerprinter import SESSION_ID, get_registry_stats

2. Update the /health endpoint to include fingerprinting session info:
   return jsonify({
     'status': 'ok',
     'service': 'Prompt Guardian API',
     'session_id': SESSION_ID,
     'session_fingerprints': get_registry_stats()['total_unique_fingerprints'],
     'session_repeats': get_registry_stats()['total_repeat_attacks']
   })

3. Add a new GET endpoint /fingerprint-registry:
   @app.route('/fingerprint-registry', methods=['GET'])
   def fingerprint_registry():
     stats = get_registry_stats()
     return jsonify(stats)

4. Update the startup print statement to include the session ID:
   print(f"Prompt Guardian API starting... Session ID: {SESSION_ID}")

Return ONLY the new import, the updated health endpoint, the new /fingerprint-registry endpoint, and the updated print statement with clear comments showing where each goes in app.py.
```

---

### Prompt 4: Display Fingerprint in content.js Overlay

Use this prompt for Sai Tej to update the extension:

```
I have an existing extension/content.js for Prompt Guardian. It contains showBlockOverlay and showWarningOverlay functions that display styled HTML overlays.

Update both overlay functions to prominently display the attack fingerprint, session ID, cluster label, and repeat attack warning.

Changes for showBlockOverlay():

1. In the overlay HTML template, find the existing .pg-score-row div (shows Risk Score % and Attack Type). AFTER this div, insert a new fingerprint row:

   <div class="pg-fingerprint-row" style="display:flex;gap:10px;align-items:center;margin-bottom:10px;padding:8px;background:#0A0E1A;border-radius:6px;border:1px solid #334155;">
     <div style="font-family:'Courier New',monospace;font-size:11px;">
       <span style="color:#64748B">ID: </span>
       <span style="color:#60A5FA;font-weight:bold;">${result.fingerprint || 'N/A'}</span>
     </div>
     <div style="color:#334155">|</div>
     <div style="font-family:'Courier New',monospace;font-size:11px;">
       <span style="color:#64748B">Session: </span>
       <span style="color:#94A3B8">${result.session_id || 'N/A'}</span>
     </div>
     <div style="color:#334155">|</div>
     <div style="font-family:'Courier New',monospace;font-size:11px;">
       <span style="color:#64748B">Cluster: </span>
       <span style="color:#8B5CF6;font-weight:bold;">${result.cluster_id || 'N/A'}</span>
     </div>
   </div>

2. If result.is_repeat is true, add a REPEAT ATTACK BANNER immediately after the fingerprint row:

   <div style="background:${result.repeat_severity === 'critical' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)'};
     border:1px solid ${result.repeat_severity === 'critical' ? '#EF4444' : '#F59E0B'};
     border-radius:6px;padding:8px 12px;margin-bottom:10px;font-size:12px;font-weight:bold;
     color:${result.repeat_severity === 'critical' ? '#EF4444' : '#F59E0B'};">
     ${result.repeat_label || ''} — This exact payload has been seen ${result.repeat_count} time(s) this session.
   </div>

3. In the .pg-layers div (shows Pattern/Semantic/Status chips), add a new chip:
   <span style="background:#4B1D8C;color:#C4B5FD;padding:4px 10px;border-radius:6px;font-size:11px;font-family:'Courier New',monospace;">
     ${result.cluster_label || 'Unknown Cluster'}
   </span>

4. Update the pg-header title dynamically:
   - If result.is_repeat and result.repeat_severity === 'critical': header text should be "🔴 REPEAT ATTACK DETECTED — {original_header_text}"
   - If result.is_repeat and result.repeat_severity === 'warning': "⚠️ REPEAT ATTACK — {original_header_text}"
   - Otherwise: original header text unchanged

5. Apply the same changes (fingerprint row, cluster chip, repeat banner if applicable) to showWarningOverlay() but with amber styling for the cluster chip instead of purple.

Return ONLY the modified template HTML sections for showBlockOverlay and showWarningOverlay with clear comments. Do not return the entire content.js file.
```

---

### Prompt 5: Add Fingerprint Gallery to dashboard.html

Use this prompt for Prithvi to update the dashboard:

```
I have an existing dashboard.html for Prompt Guardian. It already has stat cards, threat log, chart, timeline, threat intel, language breakdown, and honeypot events sections.

Add a new card for Attack Fingerprint Gallery.

Required new card HTML:
- Full-width card (grid-column: 1 / -1) with class 'card fingerprint-card'.
- Header:
    - Title: "🔏 Attack Fingerprint Registry" (h2)
    - Subtitle: "Unique attack payloads detected this session — same fingerprint = same payload"
    - Right side: session info badge showing "Session: {session_id}" in monospace amber with id 'dashboard-session-id'.
- A stats row with 3 small items (id attributes for JS population):
    - id='fp-total-unique': "Unique Fingerprints"
    - id='fp-total-repeats': "Repeat Attacks"
    - id='fp-most-repeated': "Most Repeated Count"
    Each item: large bold number (Courier New), small gray label below.
- A cluster filter row with buttons for each cluster type:
    - "All", "JB (Jailbreak)", "IO (Instruction Override)", "PE (Prompt Extraction)", "DE (Data)", "RO (Role)", "EI (Encoded)", "ML (Multilingual)"
    - Active button: background #60A5FA, color white.
    - Inactive button: background transparent, border 1px solid #334155, color #94A3B8.
    - Clicking a filter button shows only entries from that cluster family.
    - All buttons have id pattern like 'fp-filter-all', 'fp-filter-jb', etc.
- A fingerprint table with id 'fingerprint-table' and columns:
    Fingerprint | Cluster | Attack Type | Risk Score | Occurrences | Last Seen
    - Each row has a colored left border based on cluster type.
    - Repeat attacks (occurrences > 1) have an amber pulsing dot indicator.
    - If no fingerprints, show: "No attacks detected. Fingerprints appear here after the first threat is blocked."

CSS to add:
- .fingerprint-card .fp-stats { display: flex; gap: 24px; margin: 14px 0; }
- .fingerprint-card .fp-stat { text-align: center; }
- .fingerprint-card .fp-stat-num { font-family: 'Courier New', monospace; font-size: 22px; font-weight: bold; color: #60A5FA; }
- .fingerprint-card .fp-stat-label { font-size: 10px; color: #64748B; margin-top: 2px; }
- .fp-filter-row { display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 14px; }
- .fp-filter-btn { padding: 4px 12px; border-radius: 16px; cursor: pointer; font-size: 11px; transition: all 0.2s; }
- .fp-filter-btn.active { background: #60A5FA; color: white; border-color: #60A5FA; }
- .fp-fingerprint-id { font-family: 'Courier New', monospace; font-size: 12px; color: #60A5FA; font-weight: bold; }
- .fp-cluster-badge { font-family: 'Courier New', monospace; padding: 2px 8px; border-radius: 4px; font-size: 10px; background: #4B1D8C; color: #C4B5FD; }
- .fp-repeat-dot { width: 8px; height: 8px; border-radius: 50%; background: #F59E0B; display: inline-block; margin-right: 6px; animation: pulse-amber 1.5s infinite; }
- @keyframes pulse-amber { 0% { box-shadow: 0 0 0 0 rgba(245,158,11,0.7); } 70% { box-shadow: 0 0 0 6px rgba(245,158,11,0); } 100% { box-shadow: 0 0 0 0 rgba(245,158,11,0); } }
- .session-badge { font-family: 'Courier New', monospace; background: #2D1B0E; border: 1px solid #78350F; color: #F59E0B; padding: 4px 10px; border-radius: 6px; font-size: 11px; }

Return ONLY the new fingerprint card HTML and the CSS additions with clear comments.
```

---

### Prompt 6: Implement Fingerprint Dashboard Logic (dashboard.js)

Use this prompt for Prithvi to wire up the fingerprint gallery:

```
I have an existing dashboard.js for Prompt Guardian.

Add fingerprint registry visualization logic with these requirements:

1. FETCH SESSION INFO:
   - At the start of DOMContentLoaded, fetch GET http://localhost:5000/fingerprint-registry.
   - On success, update:
     - document.getElementById('dashboard-session-id').textContent = 'Session: ' + data.session_id
     - document.getElementById('fp-total-unique').textContent = data.total_unique_fingerprints
     - document.getElementById('fp-total-repeats').textContent = data.total_repeat_attacks
     - document.getElementById('fp-most-repeated').textContent = data.most_repeated_count || 0
   - On failure: show 'N/A' in all three stats (backend might be offline).

2. BUILD FINGERPRINT TABLE FROM HISTORY:
   Create a function renderFingerprintGallery(history) that:
   - Filters history to only entries where action !== 'ALLOW' (only track threats).
   - Groups by fingerprint: for each unique fingerprint string, collect:
     - fingerprint id (from history entry)
     - cluster_id and cluster_label (from history entry)
     - cluster_abbrev (first 2 chars after "CLUSTER-")
     - attack_type
     - max risk_score among all occurrences
     - total occurrence count
     - most recent timestamp
   - Sort groups by occurrence count descending, then by max risk_score descending.
   - Render each group as a table row in #fingerprint-table tbody.
   - Row structure:
     - Cell 1: fingerprint id in .fp-fingerprint-id span
     - Cell 2: cluster_id in .fp-cluster-badge span
     - Cell 3: attack_type
     - Cell 4: max risk_score + '%'
     - Cell 5: If count > 1: pulsing amber dot + count + "x". If count === 1: "1 (unique)"
     - Cell 6: most recent timestamp as toLocaleTimeString()
   - Each row gets data-cluster attribute set to cluster_abbrev (for filter functionality).

3. CLUSTER FILTER BUTTONS:
   - After rendering the table, attach click listeners to all .fp-filter-btn elements.
   - On click:
     - Remove 'active' class from all buttons.
     - Add 'active' class to clicked button.
     - Get the filter value from the button's data-filter attribute (set in HTML).
     - If filter is 'all': show all rows (remove display:none from all).
     - Otherwise: hide rows where data-cluster !== filter, show rows where it matches.

4. HANDLE MISSING FIELDS:
   - History entries from before fingerprinting was added won't have fingerprint/cluster fields.
   - Skip entries where fingerprint is undefined or null.

5. Call renderFingerprintGallery(history) after other rendering calls.

Return ONLY the new fetchFingerprintRegistry function, the renderFingerprintGallery function, the filter button logic, and all function calls, with clear comments.
```

---

### Prompt 7: Update popup.html and popup.js for Session Info

Use this prompt for Prithvi to add session context to popup:

```
I have existing popup.html and popup.js for Prompt Guardian.

Add a session identity section to the popup.

In popup.html:
- Above the existing stats grid, add a compact "session identity" bar:
  <div style="display:flex;justify-content:space-between;align-items:center;
    background:#0A0E1A;border:1px solid #1E293B;border-radius:6px;
    padding:6px 12px;margin-bottom:10px;">
    <div style="font-family:'Courier New',monospace;font-size:10px;color:#64748B;">
      Session: <span id='popup-session-id' style="color:#F59E0B">Loading...</span>
    </div>
    <div style="font-family:'Courier New',monospace;font-size:10px;color:#64748B;">
      Fingerprints: <span id='popup-fp-count' style="color:#60A5FA">0</span>
    </div>
    <div style="font-family:'Courier New',monospace;font-size:10px;color:#64748B;">
      Repeats: <span id='popup-repeat-count' style="color:#EF4444">0</span>
    </div>
  </div>

In popup.js:
- After existing rendering, fetch GET http://localhost:5000/fingerprint-registry.
- On success:
    document.getElementById('popup-session-id').textContent = data.session_id
    document.getElementById('popup-fp-count').textContent = data.total_unique_fingerprints
    document.getElementById('popup-repeat-count').textContent = data.total_repeat_attacks
- On failure:
    document.getElementById('popup-session-id').textContent = 'OFFLINE'
    document.getElementById('popup-fp-count').textContent = '—'
    document.getElementById('popup-repeat-count').textContent = '—'

Return ONLY the new HTML section and the new JS fetch block with clear comments showing insertion points.
```

---

### Prompt 8: Final Integration & Polish Review

After all files are complete, run this final review prompt in Claude:

```
Review my complete "Attack Fingerprinting" feature across these files:
1. firewall/fingerprinter.py (fingerprint generation, clustering, repeat tracking)
2. firewall/analyzer.py (fingerprinting integrated into pipeline)
3. app.py (session ID on startup, /fingerprint-registry endpoint)
4. extension/content.js (fingerprint display in overlays, repeat attack warning)
5. extension/dashboard.html + dashboard.js (fingerprint gallery, cluster filter)
6. extension/popup.html + popup.js (session identity bar)

Verify all of the following:

Backend (fingerprinter.py):
- generate_attack_fingerprint() is deterministic — same prompt always produces same fingerprint.
- Normalization (lowercase + strip) ensures "Ignore Instructions" and "ignore instructions " produce same hash.
- assign_cluster() covers all attack types and risk bands without throwing errors on unknown combinations.
- track_fingerprint() correctly increments FINGERPRINT_REGISTRY and returns the new count.
- get_repeat_status() returns correct severity ('none', 'warning', 'critical') for counts 1, 2, 3+.
- fingerprint_prompt() correctly combines all functions and returns the full dict.
- get_registry_stats() returns accurate stats from FINGERPRINT_REGISTRY.
- FINGERPRINT_REGISTRY is module-level and persists across multiple requests within same server session.
- SESSION_ID is generated ONCE at module load and never changes.
- Test block demonstrates repeat detection working correctly.

API integration:
- /analyze now returns all fingerprint fields: fingerprint, session_id, cluster_id, cluster_label, cluster_abbrev, repeat_count, is_repeat, repeat_label, repeat_severity.
- /fingerprint-registry returns session-level stats.
- /health includes session_id, session_fingerprints, session_repeats.

Extension overlay (content.js):
- Fingerprint row shows ATK-XXXXXXXX, session ID, cluster ID in monospace Courier New.
- Repeat attack banner appears ONLY when is_repeat is true.
- Banner color is red for 'critical' severity, amber for 'warning'.
- Cluster chip appears in .pg-layers with purple background.
- Header text updated with "REPEAT ATTACK" prefix when applicable.
- All new HTML uses ${result.field || 'N/A'} defensive defaults.

Dashboard fingerprint gallery:
- History entries are correctly grouped by unique fingerprint.
- Occurrence counts are accurate.
- Pulsing amber dot appears ONLY for entries with count > 1.
- Cluster filter buttons correctly show/hide table rows.
- Session ID, total unique, and total repeats fetch from /fingerprint-registry.
- Empty state message appears correctly.
- Rows with missing fingerprint fields (old history entries) are skipped gracefully.

Popup session bar:
- Session ID, fingerprint count, repeat count populate from /fingerprint-registry.
- Shows 'OFFLINE' gracefully when backend is not running.

Edge cases:
- First time any prompt is analyzed: repeat_count = 1, is_repeat = false, no banner shown.
- Second submission of identical prompt: repeat_count = 2, warning banner appears.
- Three or more identical submissions: critical banner, "Persistent attack" label.
- Server restart: FINGERPRINT_REGISTRY clears, new SESSION_ID generated, repeat counts reset.
- Unknown attack type: assigned to "UN" cluster without crashing.

If any issues are found, provide precise fixes. Otherwise confirm this feature is fully production-ready and will significantly impress judges with forensic tracking capabilities.
```
...
**Demo line for judges:**
*"Every attack Prompt Guardian detects gets a unique cryptographic fingerprint — a deterministic MD5 hash of the payload. Watch — I submit this attack: ATK-7F3A2B9C appears. Now I submit the EXACT same payload again — same fingerprint, but this time the system recognizes it as a repeat attack and escalates the warning. A third time? We call it a Persistent Attack and flag it red. This is how threat actors are tracked in real security systems — by their attack signatures, not just their IP addresses. Our fingerprints are organized into clusters — CLUSTER-JB-C01 means Jailbreak, Critical severity, first variant — making attack family analysis instant. In the dashboard, our Fingerprint Registry shows every unique payload seen this session, filterable by attack family, with repeat counts highlighted with pulsing amber indicators. We even track a Session ID so forensic analysts can correlate multiple reports from the same session. This is production-grade threat intelligence infrastructure built into a browser extension."*


-----------------------------------------------------------------------
FEATURE 10: Sound + Browser Notification
